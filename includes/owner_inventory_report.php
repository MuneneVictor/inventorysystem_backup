        ...
        try {
            ...
            if ($itemType==='device') {
                ...
                $upd=$conn->prepare("
                    ...
                    WHERE serial_number=?
                ");
                $upd->execute([$sellingPrice,$salesPerson,$serialPost]);
                $saleItemType='device';
            } else {
                ...
                $upd=$conn->prepare("UPDATE monitors SET status='Sold',selling_price=?,sold_at=NOW(),sold_by=? WHERE serial_number=?");
                $upd->execute([$sellingPrice,$salesPerson,$serialPost]);
                $saleItemType='monitors';
            }

            $sale=$conn->prepare("INSERT INTO sales (total_amount,sale_status,completed_at,sold_by,payment_status,completion_status) VALUES (?,'completed',NOW(),?,?,'Completed')");
            $sale->execute([$sellingPrice,$salesPerson,$paymentStatus]);
            ...
            $log->execute([$user_id,"Marked $itemType SN: $serialPost as sold under $ownerLabel; salesperson: {$salesUser['full_name']}; price: KES ".number_format($sellingPrice,2)."; payment: $paymentStatus; sale #$saleId"]);
