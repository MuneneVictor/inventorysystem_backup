    ...
    if (!isset($_FILES['return_proof']) || $_FILES['return_proof']['error'] !== UPLOAD_ERR_OK) {
        ...
        if (!in_array($ext, $allowed_exts)) {
            ...
            if (!$return_error) {
                ...
                if (!$return_error) {
                    $filename = 'device_return_' . $log_id . '_' . time() . '.' . $ext;
                    $destination = $upload_dir . $filename;
                    if (move_uploaded_file($file['tmp_name'], $destination)) {
                        chmod($destination, 0644);
                        $uploaded_file = '../uploads/device_returns/' . $filename;
                    } else {
    ...
    if (!$return_error && isset($uploaded_file)) {
        try {
            ...
            $details = "Returned device SN: {$log['serial_number']} from log ID {$log_id}. " .
                       ...
                       "Status set to returned. <a href='{$uploaded_file}' target='_blank'>View Photo</a>";
...
if (in_array($role, ['super_admin', 'inventory_admin', 'manager'])) {
    if ($role === 'manager') {
        ...
        $stmt = $conn->query("SELECT DISTINCT branch FROM devices_logs ORDER BY branch");
...
<head>
    ...
    <title>Device Logs | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS HDD LOGS ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        ...
        <?php if ($role === 'inventory_admin'): ?>
            <div style="margin-top:0.5rem; font-size:0.85rem; color:var(--gray-600);">
                <i class="fas fa-info-circle"></i> Showing only logs you have performed.
            </div>
        <?php endif; ?>
    </div>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value"><?= number_format(count($actions)) ?></div>
                ...
                <table>
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($logs as $log): ?>
                            ...
                            if ($log['status'] == 'instock') $statusClass = 'badge-instock';
                            ...
                            <tr>
                                ...
                                <td><small><?= date('M j, Y g:i A', strtotime($date)) ?></small></td>
                                <td>
                                    <?php if ($log['status'] == 'instock'): ?>
                                        <button class="btn btn-warning" style="padding:0.3rem 0.7rem; font-size:0.8rem;" 
                                                onclick="openReturnModal(<?= $log['id'] ?>)">
...
<div class="modal-overlay" id="returnModal">
    <div class="modal-box">
        ...
        <p style="margin-bottom:1rem; color:var(--gray-500);">Return this device to store inventory. A photo proof is required.</p>
        <form method="POST" id="returnForm" enctype="multipart/form-data">
            ...
            <div class="modal-actions">
                ...
                <button type="submit" name="return_device" class="btn btn-primary" id="returnSubmitBtn">Return</button>
            </div>
        </form>
    </div>
</div>
