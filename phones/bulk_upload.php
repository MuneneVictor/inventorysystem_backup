...
if (!in_array($role, ['super_admin', 'inventory_admin', 'manager'])) {
    die("Access denied! Only administrators can upload phones.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_FILES['excel_file'])) {
    ...
    if (!in_array($fileExtension, $allowedExtensions)) {
        ...
        try {
            ...
            $requiredColumns = ['serial_number', 'brand', 'model', 'ram', 'storage_capacity'];
            ...
            if (!empty($missingColumns)) {
                ...
                $hasConditionColumn = in_array('phone_condition', $header);
                ...
                foreach ($rows as $rowIndex => $row) {
                    ...
                    if (empty($brand)) {
                        $rowErrors[] = "Brand is empty";
                    }
                    if (empty($model)) {
                        $rowErrors[] = "Model is empty";
                    }
                    if ($ram < 1) {
                        $rowErrors[] = "RAM must be a positive integer (GB)";
                    }
                    if ($storage_capacity < 1) {
                        $rowErrors[] = "Storage capacity must be a positive integer (GB)";
                    }
                    
                    // Optional fields with defaults
                    $price = $hasPriceColumn && is_numeric($data['price']) && (float)$data['price'] > 0 ? (float)$data['price'] : null;
                    $phone_condition = $hasConditionColumn && !empty(trim($data['phone_condition'])) 
                        ? trim($data['phone_condition']) 
                        : null;
                    
                    // Validate condition
                    $validConditions = ['New', 'Ex-Uk'];
                    if ($phone_condition && !in_array($phone_condition, $validConditions)) {
                    ...
                    $stmt = $conn->prepare("SELECT serial_number FROM phones WHERE serial_number = :serial");
                    ...
                    $insert = $conn->prepare("
                        INSERT INTO phones 
                        (serial_number, brand, model, ram, storage_capacity, status, branch, added_by, price, phone_condition) 
                        VALUES 
                        (:serial_number, :brand, :model, :ram, :storage_capacity, 'instock', :branch, :added_by, :price, :phone_condition)
                    ");
                    ...
                    $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) 
                                           VALUES (:user_id, 'Bulk upload Phone', :details)");
                    $log->execute([
                        'user_id' => $added_by,
                        'details' => "Added Phone SN: $serial_number (Brand: $brand, Model: $model) via Excel upload to branch: $branch"
                    ]);

                    $addedCount++;
                }

                $success = "$addedCount phone(s) added successfully.";
                if ($duplicateCount > 0) {
...
<head>
    ...
    <title>Bulk Upload Phones | Mombasa Computers</title>
        ...
        <div class="card-body">
            <div class="info-box">
                ...
                <table>
                    ...
                    <tbody>
                        <tr><td>serial_number</td><td class="required">Required</td><td>Unique phone serial number</td><td>Alphanumeric, max 50 chars</td></tr>
                        <tr><td>brand</td><td class="required">Required</td><td>Phone brand</td><td>e.g., Apple, Samsung</td></tr>
                        <tr><td>model</td><td class="required">Required</td><td>Phone model</td><td>e.g., iPhone 15 Pro</td></tr>
                        <tr><td>ram</td><td class="required">Required</td><td>RAM size in GB</td><td>Positive integer</td></tr>
                        <tr><td>storage_capacity</td><td class="required">Required</td><td>Storage size in GB</td><td>Positive integer</td></tr>
                        <tr><td>branch</td><td class="optional">Optional</td><td>Assigned branch</td><td>KIMATHI or MOI (defaults to your branch)</td></tr>
                        <tr><td>price</td><td class="optional">Optional</td><td>Price per unit</td><td>Decimal number</td></tr>
                        <tr><td>phone_condition</td><td class="optional">Optional</td><td>Phone condition</td><td>New or Ex-Uk</td></tr>
                    </tbody>
                </table>
            </div>
            ...
            <div class="info-box">
                ...
                <div class="template-example">
                    serial_number | brand | model | ram | storage_capacity | branch | price | phone_condition<br>
                    ------------------------------------------------------------<br>
                    PH001 | Apple | iPhone 15 Pro | 8 | 256 | KIMATHI | 120000.00 | New<br>
                    PH002 | Samsung | Galaxy S24 | 12 | 512 | MOI | 150000.00 | New<br>
                    PH003 | Nokia | G22 | 4 | 128 | KIMATHI | 25000.00 | Ex-Uk
                </div>
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    document.getElementById('downloadTemplate').addEventListener('click', function(e) {
        e.preventDefault();
        const csvContent = "serial_number,brand,model,ram,storage_capacity,branch,price,phone_condition\n" +
            "PH001,Apple,iPhone 15 Pro,8,256,KIMATHI,120000.00,New\n" +
            "PH002,Samsung,Galaxy S24,12,512,MOI,150000.00,New\n" +
            "PH003,Nokia,G22,4,128,KIMATHI,25000.00,Ex-Uk";
        const blob = new Blob([csvContent], { type: 'text/csv' });
        const url = URL.createObjectURL(blob);
        const a = document.createElement('a');
        a.href = url;
        a.download = 'phone_upload_template.csv';
