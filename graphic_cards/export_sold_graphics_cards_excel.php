...
// Allow same roles as the view page (sold_graphics_cards.php)
...
// Get filters (same as sold_graphics_cards.php)
...
// Build query (same as sold_graphics_cards.php)
...
if (empty($sales)) {
    die("No sold graphic card data to export.");
}

// Create spreadsheet
$spreadsheet = new Spreadsheet();
$sheet = $spreadsheet->getActiveSheet();
$sheet->setTitle('Sold Graphic Cards');

$row = 1;

// ---- Title ----
$sheet->setCellValue('A' . $row, 'Sold Graphic Cards Report');
...
// ---- Headers ----
$headers = ['#', 'Type', 'Storage (GB)', 'Quantity', 'Selling Price (KES)', 'Total (KES)', 'Branch', 'Sold By', 'Date Sold'];
$headerRow = $row;
...
foreach ($sales as $sale) {
    ...
    $sheet->setCellValue('C' . $dataRow, $sale['storage_capacity']);
...
// ---- Output file ----
$filename = 'Sold_Graphic_Cards_' . date('Y-m-d') . '.xlsx';
