...
$search_brand = trim($_GET['brand'] ?? '');
$search_model = trim($_GET['model'] ?? '');
$search_branch = trim($_GET['branch'] ?? '');

// Build query – only show instock phones
$sql = "SELECT p.*, 
...
// Stats
$total_items = count($phones);
$total_quantity = count($phones); // each phone is 1 unit
$total_value = array_sum(array_column($phones, 'price'));
$branches = array_unique(array_column($phones, 'branch'));
?>
...
<head>
    ...
    <title>In‑Stock Phones | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS hdds_instock.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-mobile-alt"></i> In‑Stock Phones</h1>
    ...
    <div class="search-section">
        <div class="search-title"><i class="fas fa-filter"></i> Filter Phones</div>
        <form method="GET" class="search-grid">
            <div class="search-group">
                <label>Brand</label>
                <input type="text" name="brand" placeholder="e.g., Apple, Samsung" value="<?= htmlspecialchars($search_brand) ?>">
            </div>
            <div class="search-group">
                <label>Model</label>
                <input type="text" name="model" placeholder="e.g., iPhone 15, Galaxy S24" value="<?= htmlspecialchars($search_model) ?>">
            </div>
            <?php if ($role !== 'manager'): ?>
            <div class="search-group">
                <label>Branch</label>
            ...
            <div class="search-actions">
                ...
                <a href="phones_instock.php" class="btn btn-secondary"><i class="fas fa-undo"></i> Reset</a>
                <?php if (!empty($phones)): ?>
                    <a href="export_phones_excel.php?<?= http_build_query(array_merge($_GET, ['export' => '1'])) ?>" class="btn btn-excel"><i class="fas fa-file-excel"></i> Export to Excel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive">
            <?php if (empty($phones)): ?>
                <div class="empty-state">
                    ...
                    <a href="phones_instock.php" class="btn btn-primary" style="margin-top: 1rem;">
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($phones as $p): ?>
                            <tr>
                                ...
                                <td><?= (int)$p['ram'] ?> GB</td>
                                <td><?= (int)$p['storage_capacity'] ?> GB</td>
                                <td>
                                    <span class="<?= $p['branch'] == 'KIMATHI' ? 'branch-kimathi' : 'branch-moi' ?>">
                                        <?= htmlspecialchars($p['branch']) ?>
                                    </span>
                                </td>
                                <td><span class="badge"><?= htmlspecialchars($p['phone_condition'] ?? 'N/A') ?></span></td>
                                <td class="price"><?= $p['price'] !== null ? 'KES '.number_format($p['price'], 2) : '-' ?></td>
                                <td><?= htmlspecialchars($p['added_by_name'] ?? 'N/A') ?></td>
