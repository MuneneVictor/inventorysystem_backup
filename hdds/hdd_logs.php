    ...
    if ($return_qty <= 0) {
        ...
        if (!isset($_FILES['return_proof']) || $_FILES['return_proof']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($file['type'], $allowed_types)) {
                ...
                $upload_dir = __DIR__ . '/../uploads/hdd_returns/';
                if (!is_dir($upload_dir)) {
                    mkdir($upload_dir, 0777, true);
                }
                $extension = pathinfo($file['name'], PATHINFO_EXTENSION);
                $filename = 'hdd_return_' . $log_id . '_' . time() . '.' . $extension;
                $destination = $upload_dir . $filename;
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    // FIXED: Added '../' so that from /logs/ the link goes to /uploads/
                    $uploaded_file = '../uploads/hdd_returns/' . $filename;
                } else {
        ...
        if (!$return_error && isset($uploaded_file)) {
            try {
                ...
                $logStmt = $conn->prepare("SELECT * FROM hdd_logs WHERE id = ? AND status = 'pending_sale' FOR UPDATE");
                $logStmt->execute([$log_id]);
                $log = $logStmt->fetch(PDO::FETCH_ASSOC);
                if (!$log) {
                    throw new Exception("Log not found or not pending.");
                }
                if ($return_qty > $log['quantity_given']) {
                    throw new Exception("Return quantity exceeds the given quantity.");
                }

                // Lock the corresponding HDD row
                $hddStmt = $conn->prepare("SELECT * FROM hdds WHERE id = ? FOR UPDATE");
                $hddStmt->execute([$log['hdd_id']]);
                $hdd = $hddStmt->fetch(PDO::FETCH_ASSOC);
                if (!$hdd) {
                    throw new Exception("HDD record not found.");
                }

                // Update HDD quantity
                $new_hdd_qty = $hdd['quantity'] + $return_qty;
                $updateHdd = $conn->prepare("UPDATE hdds SET quantity = ?, updated_by = ?, date_updated = NOW() WHERE id = ?");
                $updateHdd->execute([$new_hdd_qty, $user_id, $hdd['id']]);

                // Update log: reduce quantity or change status
                if ($return_qty == $log['quantity_given']) {
                    // Full return – set status to 'returned'
                    $updateLog = $conn->prepare("UPDATE hdd_logs SET status = 'returned' WHERE id = ?");
                    $updateLog->execute([$log_id]);
                    $new_log_qty = $log['quantity_given'];
                } else {
                    // Partial return – reduce quantity, keep status 'pending_sale'
                    $new_log_qty = $log['quantity_given'] - $return_qty;
                    $updateLog = $conn->prepare("UPDATE hdd_logs SET quantity_given = ? WHERE id = ?");
                    $updateLog->execute([$new_log_qty, $log_id]);
                }

                // Activity log with proof file info
                $action = ($return_qty == $log['quantity_given']) ? "Returned HDD (full)" : "Returned HDD (partial)";
                $details = "Returned {$return_qty} HDD(s) ({$log['type']}, {$log['storage']}) from log ID {$log_id}. " .
                ...
                $conn->commit();
                $return_success = "HDD returned successfully! Photo uploaded.";
...
if (in_array($role, ['super_admin', 'inventory_admin'])) {
    $stmt = $conn->query("SELECT DISTINCT branch FROM hdd_logs ORDER BY branch");
...
<head>
    ...
    <title>HDD Logs | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* Same CSS as before – plus preview styles */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Total HDDs Given</div>
        </div>
        <div class="stat-card">
                                ...
                                <td>
                                    <?php if ($log['status'] == 'pending_sale'): ?>
                                        <button class="btn btn-warning" style="padding:0.3rem 0.7rem; font-size:0.8rem;" 
                                                onclick="openReturnModal(<?= $log['id'] ?>, <?= $log['quantity_given'] ?>)">
    ...
    <div class="modal-box">
        ...
        <form method="POST" id="returnForm" enctype="multipart/form-data">
            ...
            <div class="modal-actions">
                ...
                <button type="submit" name="return_hdd" class="btn btn-primary" id="returnSubmitBtn">Return</button>
            </div>
        </form>
    </div>
</div>
