...
$brandStmt = $conn->query("SELECT DISTINCT brand FROM phones WHERE status = 'instock' AND brand IS NOT NULL ORDER BY brand");
$brands = $brandStmt->fetchAll(PDO::FETCH_COLUMN);

// Build SQL query grouping by brand, model, ram, storage_capacity, phone_condition, price
$sql = "
    SELECT 
        brand,
        model,
...
$sql .= "
    GROUP BY brand, model, ram, storage_capacity, phone_condition, price
...
$unique_brands = count(array_unique(array_column($groups, 'brand')));
?>
...
<head>
    ...
    <title>Phone Price List | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS price_list.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-dollar-sign"></i> Phone Price List</h1>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value"><?= number_format($unique_brands) ?></div>
    ...
    <div class="filter-section">
        <div class="filter-title"><i class="fas fa-filter"></i> Filter Phones</div>
        <form method="GET" id="filterForm" class="filter-form">
            <div class="filter-group">
                <label>Brand</label>
                <select name="brand" onchange="autoApplyFilter()">
                ...
                <table>
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($groups as $g): ?>
                            <tr>
                                <td><?= $i++ ?></td>
                                <td><strong><?= htmlspecialchars($g['brand']) ?></strong></td>
                                <td><?= htmlspecialchars($g['model']) ?></td>
                                <td><span class="badge"><?= (int)$g['ram'] ?> GB</span></td>
                                <td><span class="badge"><?= (int)$g['storage_capacity'] ?> GB</span></td>
                                <td><span class="badge"><?= htmlspecialchars($g['phone_condition'] ?? 'N/A') ?></span></td>
                                <td class="price">
                                    <?= $g['price'] !== null ? 'KES ' . number_format($g['price'], 2) : '-' ?>
                                </td>
                                <td><span class="badge"><?= (int)$g['quantity'] ?></span></td>
                                <td><?= $g['price_updated_at'] ? date('Y-m-d H:i', strtotime($g['price_updated_at'])) : '-' ?></td>
                                <td>
                                    <?php if ($g['price'] === null): ?>
                                        <a class="btn btn-add" 
                                           href="add_price_phone.php?brand=<?= urlencode($g['brand']) ?>&model=<?= urlencode($g['model']) ?>&ram=<?= (int)$g['ram'] ?>&storage_capacity=<?= (int)$g['storage_capacity'] ?>&phone_condition=<?= urlencode($g['phone_condition'] ?? '') ?>">
                                            <i class="fas fa-plus"></i> Add Price
                                        </a>
                                    <?php else: ?>
                                        <a class="btn btn-primary" 
                                           href="update_price_phone.php?brand=<?= urlencode($g['brand']) ?>&model=<?= urlencode($g['model']) ?>&ram=<?= (int)$g['ram'] ?>&storage_capacity=<?= (int)$g['storage_capacity'] ?>&phone_condition=<?= urlencode($g['phone_condition'] ?? '') ?>&price=<?= $g['price'] ?>">
