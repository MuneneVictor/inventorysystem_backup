...
// Get filters (same as hdd_logs.php)
...
// Build query (same as hdd_logs.php)
$sql = "SELECT l.*, 
...
if (empty($logs)) {
    die("No HDD logs to export.");
}

// Create spreadsheet
$spreadsheet = new Spreadsheet();
$sheet = $spreadsheet->getActiveSheet();
$sheet->setTitle('HDD Logs');
...
// ---- Title ----
$sheet->setCellValue('A' . $row, 'HDD Logs Report');
...
// ---- Headers ----
$headers = ['#', 'Type', 'Storage', 'Quantity', 'Given To', 'Given By', 'Branch', 'Status', 'Date Given'];
$headerRow = $row;
...
foreach ($logs as $log) {
    ...
    $sheet->setCellValue('C' . $dataRow, $log['storage']);
    $sheet->setCellValue('D' . $dataRow, $log['quantity_given']);
...
$sheet->setCellValue('D' . $summaryRow, array_sum(array_column($logs, 'quantity_given')));
...
// ---- Output file ----
$filename = 'HDD_Logs_' . date('Y-m-d') . '.xlsx';
