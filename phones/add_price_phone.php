...
if (empty($brand) || empty($model) || $ram <= 0 || $storage_capacity <= 0) {
    die("Invalid group parameters.");
}

// Build condition clause for phone_condition (IS NULL if empty)
$cond_clause = $phone_condition ? '= :condition' : 'IS NULL';

// Count how many phones in this group are missing a price
...
if (!$sample) {
    die("No phones found matching this group without a price.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        if ($apply_to_all) {
            // Update all phones in this group that have no price
            $update = $conn->prepare("
                UPDATE phones
                SET price = :price,
        ...
        // Log activity
        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Added phone group price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Added price KES $price for phone group: $brand $model ($ram GB RAM, {$storage_capacity}GB storage" . ($phone_condition ? ", $phone_condition" : "") . ") – $affected phones updated"
        ]);
...
<head>
    ...
    <title>Add Phone Group Price | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== EXACT SAME CSS AS add_price.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Add Phone Group Price</h1>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Set Phone Group Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST">
                    ...
                    <?php if ($total_count > 1): ?>
                        <div class="info-note">
                            ...
                            <strong>Note:</strong> There are <strong><?= $total_count ?></strong> phones in this group that do not have a price.
