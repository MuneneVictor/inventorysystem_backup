...
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
    die("HDD ID not provided.");
}

$stmt = $conn->prepare("SELECT id, type, storage, quantity, branch, price FROM hdds WHERE id = :id");
$stmt->execute(['id' => $id]);
...
if ($hdd['price'] === null) {
    die("This HDD does not have a price. Use Add Price instead.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        $update = $conn->prepare("UPDATE hdds SET price = :price WHERE id = :id");
        $update->execute(['price' => $price, 'id' => $id]);

        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Updated HDD price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Updated price for HDD ID: $id (Type: {$hdd['type']}, Storage: {$hdd['storage']}) to KES $price"
        ]);

        $success = "Price updated successfully!";
        header("Location: hdds_instock.php");
        exit();
    }
}
?>
...
<head>
    ...
    <title>Update HDD Price | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-edit"></i> Update HDD Price</h1>
        <div class="breadcrumb">
            ...
            <?php endif; ?>
            <span> / </span>
            <a href="hdds_instock.php">In‑Stock HDDs</a>
            <span> / </span>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Update HDD Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <div class="specs-box">
                    <p><strong>ID:</strong> <?= (int)$hdd['id'] ?></p>
                    <p><strong>Type:</strong> <?= htmlspecialchars($hdd['type']) ?></p>
                    <p><strong>Storage:</strong> <?= htmlspecialchars($hdd['storage']) ?></p>
                    <p><strong>Quantity:</strong> <?= (int)$hdd['quantity'] ?></p>
                    <p><strong>Branch:</strong> <?= htmlspecialchars($hdd['branch']) ?></p>
                    <p><strong>Current Price:</strong> KES <?= number_format($hdd['price'], 2) ?></p>
                </div>

                <form method="POST">
                    <div class="form-group">
                        <label>New Price (KES) – per unit</label>
                        <input type="number" name="price" step="0.01" min="0.01" required value="<?= htmlspecialchars($hdd['price']) ?>" placeholder="Enter new price">
                    </div>
