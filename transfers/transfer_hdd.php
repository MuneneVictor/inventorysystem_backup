...
if (isset($_POST['search_hdds'])) $_SESSION['delivered_by'] = trim($_POST['delivered_by'] ?? '');

if (isset($_POST['search_hdds'])) {
    ...
    elseif (empty($delivered_by)) $error = "Please enter the name of the person delivering the HDDs.";
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer HDDs from your own branch.";
    else {
        $stmt = $conn->prepare("SELECT * FROM hdds WHERE branch = ? AND quantity > 0 ORDER BY type, storage");
        $stmt->execute([$from_branch]);
        $foundHDDs = $stmt->fetchAll(PDO::FETCH_ASSOC);
        if (empty($foundHDDs)) $error = "No HDDs found in selected branch.";
        else {
...
if (isset($_POST['transfer_hdds'])) {
    $selected = [];
    foreach ($_POST['hdds'] ?? [] as $id => $data) {
    ...
    if (empty($selected)) $error = "No HDDs selected.";
    ...
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer HDDs from your own branch.";
        ...
        try {
            foreach ($selected as $id => $qty) {
                $stmt = $conn->prepare("SELECT * FROM hdds WHERE id = ? AND branch = ? AND quantity >= ?");
                $stmt->execute([$id, $from_branch, $qty]);
                $hdd = $stmt->fetch(PDO::FETCH_ASSOC);
                if ($hdd) {
                    // Decrease from source
                    $update = $conn->prepare("UPDATE hdds SET quantity = quantity - ?, updated_by = ?, date_updated = NOW() WHERE id = ? AND branch = ?");
                    $update->execute([$qty, $user_id, $id, $from_branch]);

                    // Check if destination already has same type and storage
                    $dest = $conn->prepare("SELECT id FROM hdds WHERE type = ? AND storage = ? AND branch = ?");
                    $dest->execute([$hdd['type'], $hdd['storage'], $to_branch]);
                    $existing = $dest->fetch(PDO::FETCH_ASSOC);
                    if ($existing) {
                        $upd = $conn->prepare("UPDATE hdds SET quantity = quantity + ?, updated_by = ?, date_updated = NOW() WHERE id = ?");
                        $upd->execute([$qty, $user_id, $existing['id']]);
                    } else {
                        $ins = $conn->prepare("INSERT INTO hdds (type, quantity, storage, branch, added_by, date_added, updated_by, date_updated, price) VALUES (?, ?, ?, ?, ?, NOW(), ?, NOW(), ?)");
                        $ins->execute([$hdd['type'], $qty, $hdd['storage'], $to_branch, $user_id, $user_id, $hdd['price']]);
                    }
                    $transferredItems++;
                    $transferDetails[] = "{$hdd['type']} {$hdd['storage']} x{$qty}";
                }
            }
            $conn->commit();
            if ($transferredItems > 0) {
                $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Transfer HDDs', ?)");
                $log->execute([$user_id, "Transferred $transferredItems HDD type(s) (" . array_sum($selected) . " total items) from $from_branch to $to_branch: " . implode(', ', $transferDetails) . " (Delivered by: $delivered_by)"]);
                $success = "$transferredItems HDD type(s) transferred successfully!";
                $foundHDDs = [];
                unset($_SESSION['transfer_from_branch'], $_SESSION['transfer_to_branch'], $_SESSION['delivered_by']);
            } else $error = "No HDDs could be transferred.";
        } catch (Exception $e) {
            $conn->rollBack();
...
<head>
    ...
    <title>Transfer HDDs | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-hdd"></i> Transfer HDDs</h1>
        ...
        <form method="POST">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <button type="submit" name="search_hdds"><i class="fas fa-search"></i> Search HDDs</button>
                </div>
            </div>
        </form>

        <?php if (!empty($foundHDDs)): ?>
            <form method="POST">
                <div class="card">
                    <div class="card-header"><h2><i class="fas fa-list"></i> Available HDDs</h2></div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table>
                                <thead><tr><th>Select</th><th>Type</th><th>Storage</th><th>Available</th><th>Qty to Transfer</th></tr></thead>
                                <tbody>
                                <?php foreach ($foundHDDs as $hdd): ?>
                                    <tr>
                                        <td class="checkbox-cell"><input type="checkbox" name="hdds[<?= $hdd['id'] ?>][selected]" value="1" onchange="toggleQuantityInput(this)"></td>
                                        <td><?= htmlspecialchars($hdd['type']) ?></td>
                                        <td><?= htmlspecialchars($hdd['storage']) ?></td>
                                        <td><?= $hdd['quantity'] ?></td>
                                        <td>
                                            <input type="hidden" name="hdds[<?= $hdd['id'] ?>][max]" value="<?= $hdd['quantity'] ?>">
                                            <input type="number" name="hdds[<?= $hdd['id'] ?>][quantity]" class="quantity-input" min="1" max="<?= $hdd['quantity'] ?>" value="0" disabled onchange="validateQuantity(this)">
                                            <small>Max: <?= $hdd['quantity'] ?></small>
                                        </td>
                                    </tr>
                                <?php endforeach; ?>
                                </tbody>
                        ...
                        <button type="submit" name="transfer_hdds" style="margin-top:1rem"><i class="fas fa-arrow-right"></i> Transfer Selected</button>
                    </div>
                </div>
            </form>
        <?php endif; ?>
...
function selectAllCheckboxes(source) {
    document.querySelectorAll('input[name^="hdds["][type="checkbox"]').forEach(cb => {
