...
$id = (int)($_GET['id'] ?? 0);
if (!$id) {
    die("Charger ID not provided.");
}

$stmt = $conn->prepare("SELECT id, charger_type, charger_condition, quantity, branch, price FROM chargers WHERE id = :id");
$stmt->execute(['id' => $id]);
...
if ($charger['price'] === null) {
    die("This charger does not have a price. Use Add Price instead.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        $update = $conn->prepare("UPDATE chargers SET price = :price WHERE id = :id");
        $update->execute(['price' => $price, 'id' => $id]);

        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Updated charger price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Updated price for charger ID: $id (Type: {$charger['charger_type']}) to KES $price"
        ]);
...
<head>
    ...
    <title>Update Charger Price | Mombasa Computers</title>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Update Charger Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <div class="specs-box">
                    <p><strong>ID:</strong> <?= (int)$charger['id'] ?></p>
                    <p><strong>Type:</strong> <?= htmlspecialchars($charger['charger_type']) ?></p>
                    <p><strong>Condition:</strong> <?= htmlspecialchars(ucfirst($charger['charger_condition'])) ?></p>
                    <p><strong>Quantity:</strong> <?= (int)$charger['quantity'] ?></p>
                    <p><strong>Branch:</strong> <?= htmlspecialchars($charger['branch']) ?></p>
                    <p><strong>Current Price:</strong> KES <?= number_format($charger['price'], 2) ?></p>
                </div>

                <form method="POST">
                    <div class="form-group">
                        <label>New Price (KES) – per unit</label>
                        <input type="number" name="price" step="0.01" min="0.01" required value="<?= htmlspecialchars($charger['price']) ?>" placeholder="Enter new price">
                    </div>
