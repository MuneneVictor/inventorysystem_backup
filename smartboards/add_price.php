...
// Count how many smartboards in this group are missing a price
$countStmt = $conn->prepare("
    ...
    WHERE model = :model 
      ...
      AND price IS NULL
");
$countStmt->execute(['model' => $model, 'size' => $size]);
...
$sampleStmt = $conn->prepare("
    ...
    LIMIT 1
");
$sampleStmt->execute(['model' => $model, 'size' => $size]);
$sample = $sampleStmt->fetch(PDO::FETCH_ASSOC);

if (!$sample) {
    die("No smartboards found matching this group without a price.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        if ($apply_to_all) {
            // Update all smartboards in this group that have no price
            $update = $conn->prepare("
                UPDATE smartboards
                SET price = :price
                WHERE model = :model
            ...
            $firstStmt = $conn->prepare("
                ...
                LIMIT 1
            ");
            $firstStmt->execute(['model' => $model, 'size' => $size]);
        ...
        // Log activity
        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Added smartboard group price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Added price KES $price for smartboard group: $model ($size inch) – $affected smartboards updated"
        ]);
...
<head>
    ...
    <title>Add Smartboard Group Price | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS add_price_printer_group.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Add Smartboard Group Price</h1>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Set Smartboard Group Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST">
                    ...
                    <?php if ($total_count > 1): ?>
                        <div class="info-note">
                            ...
                            <strong>Note:</strong> There are <strong><?= $total_count ?></strong> smartboards in this group that do not have a price.
