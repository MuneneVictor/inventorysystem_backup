...
$return_error = '';
$return_success = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['return_ram_ssd'])) {
    ...
    if ($return_qty <= 0) {
        ...
        if (!isset($_FILES['return_proof']) || $_FILES['return_proof']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($file['type'], $allowed_types)) {
                ...
                $upload_dir = __DIR__ . '/../uploads/ram_ssd_returns/';
                if (!is_dir($upload_dir)) {
                    mkdir($upload_dir, 0777, true);
                }
                $extension = pathinfo($file['name'], PATHINFO_EXTENSION);
                $filename = 'ram_ssd_return_' . $log_id . '_' . time() . '.' . $extension;
                $destination = $upload_dir . $filename;
                if (move_uploaded_file($file['tmp_name'], $destination)) {
                    // From this file (in /rams_ssds/), we need to go up one level to reach /uploads/
                    $uploaded_file = '../uploads/ram_ssd_returns/' . $filename;
                } else {
        ...
        if (!$return_error && isset($uploaded_file)) {
            try {
                ...
                $logStmt = $conn->prepare("SELECT * FROM rams_ssds_logs WHERE id = ? AND status = 'pending_sale' FOR UPDATE");
                ...
                // Lock the corresponding RAM/SSD row
                $stockStmt = $conn->prepare("SELECT * FROM rams_ssds WHERE id = ? FOR UPDATE");
                $stockStmt->execute([$log['ram_ssd_id']]);
                ...
                $new_stock_qty = $stock['quantity'] + $return_qty;
                $updateStock = $conn->prepare("UPDATE rams_ssds SET quantity = ?, updated_by = ?, date_updated = NOW() WHERE id = ?");
                $updateStock->execute([$new_stock_qty, $user_id, $stock['id']]);

                // Update log: reduce quantity or change status
                if ($return_qty == $log['quantity_given']) {
                    // Full return – set status to 'returned'
                    $updateLog = $conn->prepare("UPDATE rams_ssds_logs SET status = 'returned' WHERE id = ?");
                    ...
                    $updateLog = $conn->prepare("UPDATE rams_ssds_logs SET quantity_given = ? WHERE id = ?");
                ...
                $conn->commit();
                $return_success = "RAM/SSD returned successfully! Photo uploaded.";
...
<div class="main-content">
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Total Items Given</div>
        </div>
        <div class="stat-card">
...
<div class="modal-overlay" id="returnModal">
    <div class="modal-box">
        <h3><i class="fas fa-undo-alt"></i> Return RAM/SSD</h3>
        ...
        <form method="POST" id="returnForm" enctype="multipart/form-data">
            ...
            <div class="modal-actions">
                ...
                <button type="submit" name="return_ram_ssd" class="btn btn-primary">Return</button>
            </div>
        </form>
    </div>
</div>
