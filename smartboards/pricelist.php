...
// Build SQL query grouping by model, size_inches, price
$sql = "
    SELECT 
        model,
        size_inches,
...
$sql .= "
    GROUP BY model, size_inches, price
    ORDER BY model, size_inches
";
...
<head>
    ...
    <style>
        /* ===== SAME CSS AS price_list_printers.php ===== */
        :root {
            --primary: #1a4b2a;
        ...
        <div class="table-responsive">
                        ...
                        <?php $i = 1; foreach ($groups as $g): ?>
                            <tr>
                                ...
                                <td>
                                    <?php if ($g['price'] === null): ?>
                                        <a class="btn btn-add" 
                                           href="add_price.php?model=<?= urlencode($g['model']) ?>&size=<?= (int)$g['size_inches'] ?>">
                                            <i class="fas fa-plus"></i> Add Price
                                        </a>
                                    <?php else: ?>
                                        <a class="btn btn-primary" 
                                           href="update_price.php?model=<?= urlencode($g['model']) ?>&size=<?= (int)$g['size_inches'] ?>&price=<?= $g['price'] ?>">
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>No smartboards found matching your criteria.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
