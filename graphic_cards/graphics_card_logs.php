...
$return_error = '';
$return_success = '';
if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['return_graphic'])) {
    ...
    if ($return_qty <= 0) {
        ...
        if (!isset($_FILES['return_proof']) || $_FILES['return_proof']['error'] !== UPLOAD_ERR_OK) {
            ...
            if (!in_array($ext, $allowed_exts)) {
                ...
                if (!$return_error) {
                    ...
                    $upload_dir = __DIR__ . '/../uploads/graphic_returns/';
                    ...
                    if (!$return_error) {
                        $filename = 'graphic_return_' . $log_id . '_' . time() . '.' . $ext;
                        $destination = $upload_dir . $filename;
                        // Try to move the uploaded file
                        if (move_uploaded_file($file['tmp_name'], $destination)) {
                            // Set file permissions (optional)
                            chmod($destination, 0644);
                            $uploaded_file = '../uploads/graphic_returns/' . $filename;
                        } else {
        ...
        if (!$return_error && isset($uploaded_file)) {
            try {
                ...
                $logStmt = $conn->prepare("SELECT * FROM graphic_cards_logs WHERE id = ? AND status = 'pending_sale' FOR UPDATE");
                ...
                // Lock the corresponding graphic card row
                $cardStmt = $conn->prepare("SELECT * FROM graphic_cards WHERE id = ? FOR UPDATE");
                $cardStmt->execute([$log['graphic_card_id']]);
                $card = $cardStmt->fetch(PDO::FETCH_ASSOC);
                if (!$card) {
                    throw new Exception("Graphic card record not found.");
                }

                // Update graphic card quantity
                $new_card_qty = $card['quantity'] + $return_qty;
                $updateCard = $conn->prepare("UPDATE graphic_cards SET quantity = ? WHERE id = ?");
                $updateCard->execute([$new_card_qty, $card['id']]);

                // Update log: reduce quantity or change status
                if ($return_qty == $log['quantity']) {
                    // Full return – set status to 'returned'
                    $updateLog = $conn->prepare("UPDATE graphic_cards_logs SET status = 'returned' WHERE id = ?");
                    ...
                    $updateLog = $conn->prepare("UPDATE graphic_cards_logs SET quantity = ? WHERE id = ?");
                    $updateLog->execute([$new_log_qty, $log_id]);
                }

                // Activity log with proof file info
                $action = ($return_qty == $log['quantity']) ? "Returned Graphic Card (full)" : "Returned Graphic Card (partial)";
                $details = "Returned {$return_qty} graphic card(s) ({$log['type']}, {$log['storage_capacity']}GB) from log ID {$log_id}. " .
                ...
                $conn->commit();
                $return_success = "Graphic card returned successfully! Photo uploaded.";

                // Clear output buffer and redirect
                ob_end_clean();
                header("Location: graphics_card_logs.php?success=1");
                exit;
...
if (isset($_GET['success'])) {
    $return_success = "Graphic card returned successfully!";
}
...
if ($search) {
    $sql .= " AND (l.type LIKE :search OR l.storage_capacity LIKE :search OR u_given_to.full_name LIKE :search OR u_given_by.full_name LIKE :search)";
...
if (in_array($role, ['super_admin', 'inventory_admin'])) {
    $stmt = $conn->query("SELECT DISTINCT branch FROM graphic_cards_logs ORDER BY branch");
...
<head>
    ...
    <title>Graphic Card Logs | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-history"></i> Graphic Card Logs</h1>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-label">Total Cards Given</div>
        </div>
        <div class="stat-card">
    ...
    <div class="search-section">
        ...
        <form method="GET" class="search-grid">
            ...
            <div class="search-actions">
                ...
                <a href="graphic_card_logs.php" class="btn btn-secondary"><i class="fas fa-undo"></i> Reset</a>
                <?php if (!empty($logs)): ?>
                    <a href="export_graphic_card_logs_excel.php?<?= http_build_query(array_merge($_GET, ['export' => '1'])) ?>" class="btn btn-excel"><i class="fas fa-file-excel"></i> Export to Excel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive">
            <?php if (empty($logs)): ?>
                <div class="empty-state">
                    ...
                    <p>No graphic card logs found matching your criteria.</p>
                    <a href="graphic_card_logs.php" class="btn btn-primary" style="margin-top: 1rem;">
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($logs as $log): ?>
                            ...
                            <tr>
                                ...
                                <td><?= htmlspecialchars($log['storage_capacity']) ?> GB</td>
...
<div class="modal-overlay" id="returnModal">
    <div class="modal-box">
        <h3><i class="fas fa-undo-alt"></i> Return Graphic Card</h3>
        ...
        <form method="POST" id="returnForm" enctype="multipart/form-data">
            ...
            <div class="modal-actions">
                ...
                <button type="submit" name="return_graphic" class="btn btn-primary" id="returnSubmitBtn">Return</button>
            </div>
        </form>
    </div>
</div>
