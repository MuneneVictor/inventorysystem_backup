...
// Fetch distinct models for dropdown
$modelStmt = $conn->query("SELECT DISTINCT model FROM ups WHERE status = 'instock' AND model IS NOT NULL ORDER BY model");
$models = $modelStmt->fetchAll(PDO::FETCH_COLUMN);
$conditions = ['New', 'Ex-UK', 'Refurbished'];

// Build SQL query grouping by model, capacity, ups_condition, price
$sql = "
    SELECT 
        model,
        capacity,
...
if ($filter_condition) {
    $sql .= " AND ups_condition = :condition";
    $params['condition'] = $filter_condition;
}

$sql .= "
    GROUP BY model, capacity, ups_condition, price
    ORDER BY model, capacity
";
...
$unique_models = count(array_unique(array_column($groups, 'model')));
?>
...
<head>
    ...
    <title>UPS Price List | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-dollar-sign"></i> UPS Price List</h1>
    ...
    <div class="filter-section">
        ...
        <form method="GET" id="filterForm" class="filter-form">
            <div class="filter-group">
                <label>Model</label>
                <select name="model" onchange="autoApplyFilter()">
                    <option value="">-- All Models --</option>
                    <?php foreach ($models as $m): ?>
                        <option value="<?= htmlspecialchars($m) ?>" <?= $filter_model == $m ? 'selected' : '' ?>><?= htmlspecialchars($m) ?></option>
                    <?php endforeach; ?>
                </select>
            </div>
            <div class="filter-group">
        ...
        <div class="table-responsive">
                ...
                <table>
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($groups as $g): ?>
                            <tr>
                                <td><?= $i++ ?></td>
                                <td><strong><?= htmlspecialchars($g['model']) ?></strong></td>
                                <td><?= (int)$g['capacity'] ?> VA</td>
                                <td><span class="badge"><?= htmlspecialchars($g['ups_condition'] ?? 'New') ?></span></td>
                                <td class="price">
                                ...
                                <td>
                                    <?php if ($g['price'] === null): ?>
                                        <a class="btn btn-add" 
                                           href="add_price_ups_group.php?model=<?= urlencode($g['model']) ?>&capacity=<?= (int)$g['capacity'] ?>&ups_condition=<?= urlencode($g['ups_condition'] ?? 'New') ?>">
                                            <i class="fas fa-plus"></i> Add Price
                                        </a>
                                    <?php else: ?>
                                        <a class="btn btn-primary" 
                                           href="update_price_ups_group.php?model=<?= urlencode($g['model']) ?>&capacity=<?= (int)$g['capacity'] ?>&ups_condition=<?= urlencode($g['ups_condition'] ?? 'New') ?>&price=<?= $g['price'] ?>">
            ...
            <?php else: ?>
                <div class="empty-state">
                    ...
                    <p>No UPS units found matching your criteria.</p>
                </div>
            <?php endif; ?>
        </div>
    </div>
