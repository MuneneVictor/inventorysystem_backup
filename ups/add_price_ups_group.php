...
// Count how many UPS in this group are missing a price
...
if (!$sample) {
    die("No UPS found matching this group without a price.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        if ($apply_to_all) {
            // Update all UPS in this group that have no price
            $update = $conn->prepare("
                UPDATE ups
                SET price = :price,
            ...
            if ($serial) {
                ...
                ");
                $update->execute(['price' => $price, 'serial' => $serial]);
                $affected = 1;
            } else {
                $affected = 0;
            }
        ...
        // Log activity
        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Added UPS group price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Added price KES $price for UPS group: $model ($capacity VA" . ($ups_condition ? ", $ups_condition" : "") . ") – $affected units updated"
        ]);
...
<head>
    ...
    <title>Add UPS Group Price | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Add UPS Group Price</h1>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Set UPS Group Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST">
                    ...
                    <?php if ($total_count > 1): ?>
                        <div class="info-note">
                            ...
                            <strong>Note:</strong> There are <strong><?= $total_count ?></strong> UPS units in this group that do not have a price.
                            You can apply this price to all of them or just one.
                        </div>
