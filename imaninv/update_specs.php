...
$q=$conn->prepare("SELECT * FROM `iman_inventory_items` WHERE id=? LIMIT 1");$q->execute([$id]);$item=$q->fetch(PDO::FETCH_ASSOC);
if(!$item){http_response_code(404);die("The selected Iman Inventory item could not be found. It may have been removed or the link may be outdated.");}
$error='';$success='';
...
if($_SERVER['REQUEST_METHOD']==='POST' && $error==='') {
    ...
    if(
        ...
        try {
            ...
            $u=$conn->prepare("UPDATE `iman_inventory_items`
...
</style></head><body><?php include __DIR__.'/../includes/sidebar.php';?><div class="main-content"><div class="page-header"><h1><i class="fas fa-microchip"></i> Update Item Specs <span class="serial-code"><?=safe($item['serial_number'] ?: '#'.$id)?></span></h1><div class="breadcrumb"><a href="../dashboard/superadmindashboard.php"><i class="fas fa-home"></i> Dashboard</a> / <a href="overview.php">Iman Inventory</a> / Update Specs</div><div class="page-help">The item you selected is already loaded. Enter only the hardware details that changed. Leaving a field blank keeps its current value.</div></div>
