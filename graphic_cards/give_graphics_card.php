...
if (!in_array($_SESSION['role'], ['super_admin', 'inventory_admin'])) {
    die("ACCESS DENIED. Only Inventory Admin or Super Admin can give out Graphic Cards.");
}
...
if ($selected_branch) {
    // Fetch available Graphic Cards stock (quantity > 0)
    $stockStmt = $conn->prepare("SELECT * FROM graphic_cards WHERE branch = ? AND quantity > 0 AND status = 'instock' ORDER BY type, storage_capacity");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$card_id || $quantity_given <= 0 || !$given_to || !$branch) {
        $error = "All fields are required.";
    } else {
        try {
            $conn->beginTransaction();

            // Get current Graphic Card item with FOR UPDATE to lock the row
            $itemStmt = $conn->prepare("SELECT * FROM graphic_cards WHERE id = ? AND branch = ? AND status = 'instock' FOR UPDATE");
            $itemStmt->execute([$card_id, $branch]);
            $item = $itemStmt->fetch(PDO::FETCH_ASSOC);
            if (!$item) {
                throw new Exception("Graphic Card item not found or not in stock.");
            }
            ...
            $update = $conn->prepare("UPDATE graphic_cards SET quantity = ? WHERE id = ?");
            $update->execute([$new_qty, $card_id]);

            // Insert into graphic_cards_logs
            $log = $conn->prepare("
                INSERT INTO graphic_cards_logs 
                (graphic_card_id, type, storage_capacity, quantity, given_to, given_by, branch, status) 
                VALUES (?, ?, ?, ?, ?, ?, ?, 'pending_sale')
            ");
            $log->execute([
                $item['id'],
            ...
            $activity = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Give out Graphic Card', ?)");
            $activity->execute([
                $user_id,
                "Gave {$quantity_given} graphic card(s) ({$item['type']}, {$item['storage_capacity']}GB) to {$sales_name} in {$branch} branch"
            ]);

            $conn->commit();
            $success = "Successfully gave out {$quantity_given} graphic card unit(s).";
...
<head>
    ...
    <title>Give Out Graphic Card | Mombasa Computers</title>
...
<div class="main-content">
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header"><h2><i class="fas fa-exchange-alt"></i> Select Branch & Graphic Card</h2></div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <?php elseif (empty($stocks)): ?>
                    <div class="alert alert-error">No graphic cards available in stock for <?= htmlspecialchars($selected_branch) ?> branch.</div>
                ...
                <?php else: ?>
                    <form method="POST">
                        ...
                        <div class="form-group">
                            <label>Select Graphic Card</label>
                            <select name="card_id" required>
                                <option value="">-- Choose Graphic Card --</option>
                                <?php foreach ($stocks as $s): ?>
                                    <option value="<?= $s['id'] ?>">
                                        <?= htmlspecialchars($s['type']) ?> - <?= $s['storage_capacity'] ?>GB (Available: <?= $s['quantity'] ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
