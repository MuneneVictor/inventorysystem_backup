...
$stmt=$conn->prepare("SELECT * FROM `iman_hustle_items` WHERE id=? LIMIT 1");$stmt->execute([$id]);$item=$stmt->fetch(PDO::FETCH_ASSOC);
if(!$item){http_response_code(404);die("This Iman\'s Hustle item could not be found. It may have been removed or the link may be outdated.");}

$original=$item;$error='';$success='';
if($_SERVER['REQUEST_METHOD']==='POST'){
    try{
        $item_type=in_array($_POST['item_type']??'',['Device','Monitor'],true)?$_POST['item_type']:'Device';$asset_id=nullv($_POST['asset_id']??'');$manufacturer=nullv($_POST['manufacturer']??'');$model_name=nullv($_POST['model_name']??'');$form_factor=nullv($_POST['form_factor']??'');$processor=nullv($_POST['processor']??'');$ram=nullv($_POST['ram']??'');$storage=nullv($_POST['storage']??'');$serial_number=nullv($_POST['serial_number']??'');$grade=nullv($_POST['grade']??'');$buying_price=numv($_POST['buying_price']??'');$planned_selling_price=numv($_POST['planned_selling_price']??'');$location=locv($_POST['location']??'');$notes=nullv($_POST['notes']??'');
        $conn->beginTransaction();
        $update=$conn->prepare("UPDATE `iman_hustle_items` SET `item_type`=:item_type,`asset_id`=:asset_id,`manufacturer`=:manufacturer,`model_name`=:model_name,`form_factor`=:form_factor,`processor`=:processor,`ram`=:ram,`storage`=:storage,`serial_number`=:serial_number,`grade`=:grade,`buying_price`=:buying_price,`planned_selling_price`=:planned_selling_price,`location`=:location,`notes`=:notes WHERE id=:id");
        ...
        try{
            ...
            $log->execute([$user_id,"Updated Iman\'s Hustle item ".$serialLabel."."]);
...
<div class="breadcrumb"><a href="../dashboard/superadmindashboard.php"><i class="fas fa-home"></i> Dashboard</a> / <a href="overview.php">Iman's Hustle</a> / Edit Item</div>
...
<div class="info-box">
<div class="info-item"><small>Inventory</small><strong>Iman's Hustle</strong></div>
