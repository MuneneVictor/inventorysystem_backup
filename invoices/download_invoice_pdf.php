...
if (!$invoice_id) die("Invalid invoice.");
...
<head>
    ...
    <style>
        ...
        .details .payment-status-label {
            font-weight: 700;
            color: #000000;
            font-size: 12px;
            text-transform: uppercase;
        }
        .details .payment-status-value {
            font-weight: 700;
            font-size: 14px;
            text-transform: uppercase;
        }
        .details .payment-status-value.paid { color: #16a34a; }
        .details .payment-status-value.partial { color: #c2410c; }
        .details .payment-status-value.unpaid { color: #dc2626; }
        .items-table { 
            width: 100%; 
            border-collapse: collapse; 
            margin: 12px 0; 
        ...
        <table>
            <tr>
                <td style="width:55%;">
                    ...
                    ' . (!empty($invoice['client_box']) ? '<div class="client-detail">' . htmlspecialchars($invoice['client_box']) . '</div>' : '') . '
                    ' . (!empty($invoice['client_phone']) ? '<div class="client-detail">Phone: ' . htmlspecialchars($invoice['client_phone']) . '</div>' : '') . '
                    ' . (!empty($invoice['client_email']) ? '<div class="client-detail">Email: ' . htmlspecialchars($invoice['client_email']) . '</div>' : '') . '
                </td>
                <td style="width:45%; text-align:right;">
                    <div><span class="label">INVOICE NUMBER:</span> <span class="invoice-number">' . htmlspecialchars($invoice['invoice_number']) . '</span></div>
                    <div style="margin-top:3px;"><span class="label">INVOICE DATE:</span> <span class="date-value">' . date('M d, Y', strtotime($invoice['invoice_date'])) . '</span></div>
                    <div style="margin-top:3px;"><span class="label">PAYMENT DUE:</span> <span class="payment-due">' . date('M d, Y', strtotime($invoice['payment_due_date'])) . '</span></div>
                    <div style="margin-top:3px;">
                        <span class="payment-status-label">STATUS:</span> 
                        <span class="payment-status-value ' . $invoice['payment_status'] . '">
...
if ($invoice['amount_paid'] > 0) {
    $html .= '<div class="payment-summary">
        <p><span class="paid-label">Amount Paid:</span> <span class="paid-amount">KES ' . number_format($invoice['amount_paid'], 2) . '</span></p>
        <p><span class="balance-label">Balance Due:</span> <span class="balance-amount">KES ' . number_format($invoice['balance_due'], 2) . '</span></p>
    </div>';
}

if (!empty($invoice['notes'])) {
    $html .= '<div class="notes"><strong>Notes:</strong> ' . nl2br(htmlspecialchars($invoice['notes'])) . '</div>';
}

$html .= '<div class="footer-note">Thank you for your business!</div>
</body>
</html>';
