...
$sql = "SELECT p.*, u.full_name AS sold_by_name
        FROM phones p
...
if ($date_from) {
    $sql .= " AND DATE(p.date_sold) >= :date_from";
    $params['date_from'] = $date_from;
}
if ($date_to) {
    $sql .= " AND DATE(p.date_sold) <= :date_to";
    $params['date_to'] = $date_to;
}
if (in_array($role, ['super_admin', 'inventory_admin']) && !empty($filter_salesperson)) {
    $sql .= " AND p.sold_by = :salesperson";
...
$total_revenue = array_sum(array_column($sales, 'selling_price'));
...
<head>
    ...
    <title>Sold Phones | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS sold_hdds.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-mobile-alt"></i> Sold Phones</h1>
    ...
    <div class="search-section">
        ...
        <form method="GET" class="search-grid">
            <div class="search-group">
                <label>Brand</label>
                <input type="text" name="brand" placeholder="e.g., Apple" value="<?= htmlspecialchars($search_brand) ?>">
            </div>
            <div class="search-group">
                <label>Model</label>
                <input type="text" name="model" placeholder="e.g., iPhone 15" value="<?= htmlspecialchars($search_model) ?>">
            </div>
            <?php if ($role !== 'manager'): ?>
            <div class="search-group">
                <label>Branch</label>
            ...
            <div class="search-actions">
                ...
                <a href="sold_phones.php" class="btn btn-secondary"><i class="fas fa-undo"></i> Reset</a>
                <?php if (!empty($sales)): ?>
                    <a href="export_sold_phones_excel.php?<?= http_build_query(array_merge($_GET, ['export' => '1'])) ?>" class="btn btn-excel"><i class="fas fa-file-excel"></i> Export to Excel</a>
                <?php endif; ?>
            </div>
        </form>
    </div>
        ...
        <div class="table-responsive">
            <?php if (empty($sales)): ?>
                <div class="empty-state">
                    ...
                    <p>No sold phones found matching your criteria.</p>
                    <a href="sold_phones.php" class="btn btn-primary" style="margin-top: 1rem;">
                ...
                <table>
                    ...
                    <tbody>
                        <?php $i = 1; foreach ($sales as $s): ?>
                            <tr>
                                <td><?= $i++ ?></td>
                                <td><code><?= htmlspecialchars($s['serial_number']) ?></code></td>
                                <td><strong><?= htmlspecialchars($s['brand']) ?></strong></td>
                                <td><?= htmlspecialchars($s['model']) ?></td>
                                <td><?= (int)$s['ram'] ?> GB</td>
                                <td><?= (int)$s['storage_capacity'] ?> GB</td>
                                <td class="price"><?= $s['price'] ? 'KES '.number_format($s['price'], 2) : '-' ?></td>
                                ...
                                <td><small><?= !empty($s['date_sold']) ? date('M j, Y g:i A', strtotime($s['date_sold'])) : 'N/A' ?></small></td>
                                <td>
                                    <div class="action-links">
                                        <a href="view_phone.php?sn=<?= urlencode($s['serial_number']) ?>" class="action-link">
