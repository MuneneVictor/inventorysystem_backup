...
// Fetch UPS details with added_by and sold_by names
$stmt = $conn->prepare("
    SELECT u.*, 
...
if ($ups['status'] === 'sold') {
    $sold_info = [
        'sold_by_name' => $ups['sold_by_name'] ?? 'Unknown',
        'sold_at'      => $ups['date_sold'],
        'selling_price' => $ups['selling_price'],
        'customer_name' => 'Walk-in Customer' // No customer field in ups table
    ];
}
?>
<!DOCTYPE html>
...
<head>
    ...
    <title>View UPS | <?= htmlspecialchars($ups['serial_number']) ?></title>
...
<div class="main-content">
    ...
    <div class="page-header">
        <h1>
            ...
            UPS Details
            <span class="serial-code"><?= htmlspecialchars($ups['serial_number']) ?></span>
        </h1>
    ...
    <div class="result-card">
        <div class="card-header">
            ...
            <span class="status-badge <?= $ups['status'] == 'instock' ? 'status-instock' : 'status-sold' ?>">
                <i class="fas <?= $ups['status'] == 'instock' ? 'fa-check-circle' : 'fa-times-circle' ?>"></i>
                <?= $ups['status'] == 'instock' ? 'In Stock' : 'Sold' ?>
            </span>
        </div>
        <div class="card-body">
            <div class="info-grid">
                <div class="info-item">
                    ...
                    <div class="info-value"><code><?= htmlspecialchars($ups['serial_number']) ?></code></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Model</div>
                    <div class="info-value"><strong><?= htmlspecialchars($ups['model'] ?? '-') ?></strong></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Capacity</div>
                    <div class="info-value"><?= (int)$ups['capacity'] ?> VA</div>
                </div>
                <div class="info-item">
                    <div class="info-label">Branch</div>
                    <div class="info-value">
                        <span style="color: <?= $ups['branch'] == 'KIMATHI' ? '#059669' : '#3b82f6' ?>">
                            <i class="fas <?= $ups['branch'] == 'KIMATHI' ? 'fa-building' : 'fa-store' ?>"></i>
                            <?= htmlspecialchars($ups['branch']) ?>
                        </span>
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Condition</div>
                    <div class="info-value"><?= htmlspecialchars($ups['ups_condition'] ?? 'New') ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Price (KES)</div>
                    <div class="info-value">
                        <?= $ups['price'] !== null ? 'KES ' . number_format($ups['price'], 2) : '—' ?>
                    </div>
                </div>
                <div class="info-item">
                    <div class="info-label">Added By</div>
                    <div class="info-value"><?= htmlspecialchars($ups['added_by_name'] ?? 'Unknown') ?></div>
                </div>
                <div class="info-item">
                    <div class="info-label">Date Added</div>
                    <div class="info-value"><?= date('M j, Y H:i', strtotime($ups['date_added'])) ?></div>
                </div>
            </div>
        </div>
    </div>
