...
$ups_condition = trim($_GET['ups_condition'] ?? 'New');
$current_price = isset($_GET['price']) ? (float) $_GET['price'] : null;

if (empty($model) || $capacity <= 0 || $current_price === null) {
    die("Invalid group parameters.");
}

// Build condition clause for ups_condition
$cond_clause = $ups_condition ? '= :condition' : 'IS NULL';

// Count UPS in this group that have this price
$countStmt = $conn->prepare("
    SELECT COUNT(*) 
    FROM ups 
    WHERE model = :model 
      AND capacity = :capacity 
      AND ups_condition $cond_clause
      AND price = :price
      AND status = 'instock'
");
$params = [
...
if ($ups_condition) $params['condition'] = $ups_condition;
$countStmt->execute($params);
$total_count = $countStmt->fetchColumn();

// Fetch one sample UPS to display specs
...
if (!$sample) {
    die("No UPS found matching this group with the current price.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($new_price) || $new_price <= 0) {
        ...
        if ($apply_to_all) {
            // Update all UPS in this group that have the current price
            $update = $conn->prepare("
                UPDATE ups
                SET price = :new_price,
            ...
            if ($ups_condition) $updateParams['condition'] = $ups_condition;
            $update->execute($updateParams);
            $affected = $update->rowCount();
        } else {
            // Update only the first UPS in this group
        ...
        // Log activity
        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Updated UPS group price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Updated price from KES $current_price to KES $new_price for UPS group: $model ($capacity VA" . ($ups_condition ? ", $ups_condition" : "") . ") – $affected units updated"
        ]);

        header("Location: price_list_ups.php");
        exit();
    }
}
...
<head>
    ...
    <title>Update UPS Group Price | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS update_price.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-edit"></i> Update UPS Group Price</h1>
        <div class="breadcrumb">
            ...
            <?php endif; ?>
            <span> / </span>
            <a href="price_list_ups.php">UPS Price List</a>
            <span> / </span>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Update UPS Group Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST">
                    <div class="specs-box">
                        ...
                        <p><strong>Capacity:</strong> <?= (int)$sample['capacity'] ?> VA</p>
                        <p><strong>Condition:</strong> <?= htmlspecialchars($sample['ups_condition'] ?? 'New') ?></p>
                    ...
                    <?php if ($total_count > 1): ?>
                        <div class="info-note">
                            ...
                            <strong>Note:</strong> There are <strong><?= $total_count ?></strong> UPS units in this group that have this price.
                        ...
                        <div class="checkbox-group">
                            ...
                            <label for="apply_to_all">Apply this price to ALL <?= $total_count ?> UPS units with same specifications</label>
                        </div>
                    <?php endif; ?>
