...
if (!in_array($_SESSION['role'], ['super_admin', 'inventory_admin'])) {
    die("ACCESS DENIED. Only Inventory Admin or Super Admin can give out HDDs.");
}
...
if ($selected_branch) {
    // Fetch available HDD stock (quantity > 0)
    $stockStmt = $conn->prepare("SELECT * FROM hdds WHERE branch = ? AND quantity > 0 ORDER BY type, storage");
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    $hdd_id = (int) ($_POST['hdd_id'] ?? 0);
    $quantity_given = (int) ($_POST['quantity'] ?? 0);
    $given_to = (int) ($_POST['given_to'] ?? 0);
    $branch = trim($_POST['branch'] ?? '');

    if (!$hdd_id || $quantity_given <= 0 || !$given_to || !$branch) {
        $error = "All fields are required.";
    } else {
        try {
            $conn->beginTransaction();

            // Get current HDD item with FOR UPDATE to lock the row
            $itemStmt = $conn->prepare("SELECT * FROM hdds WHERE id = ? AND branch = ? FOR UPDATE");
            $itemStmt->execute([$hdd_id, $branch]);
            $item = $itemStmt->fetch(PDO::FETCH_ASSOC);
            if (!$item) {
                throw new Exception("HDD item not found.");
            }
            ...
            $update = $conn->prepare("UPDATE hdds SET quantity = ?, updated_by = ?, date_updated = NOW() WHERE id = ?");
            $update->execute([$new_qty, $user_id, $hdd_id]);

            // Insert into hdd_logs
            $log = $conn->prepare("INSERT INTO hdd_logs (hdd_id, type, storage, quantity_given, given_to, given_by, branch, status) VALUES (?, ?, ?, ?, ?, ?, ?, 'pending_sale')");
            $log->execute([
                $item['id'],
                $item['type'],
                $item['storage'],
            ...
            $activity = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Give out HDD', ?)");
            $activity->execute([
                $user_id,
                "Gave {$quantity_given} HDD(s) ({$item['type']}, {$item['storage']}) to {$sales_name} in {$branch} branch"
            ]);

            $conn->commit();
            $success = "Successfully gave out {$quantity_given} HDD unit(s).";
...
<head>
    ...
    <title>Give Out HDD | Mombasa Computers</title>
    <link href="https://fonts.googleapis.com/css2?family=Inter:opsz,wght@400;500;600;700&display=swap" rel="stylesheet">
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0-beta3/css/all.min.css">
    <style>
        /* Same base styles as give_ram.php – consistent with inventory system */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header"><h2><i class="fas fa-exchange-alt"></i> Select Branch & HDD</h2></div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <?php elseif (empty($stocks)): ?>
                    <div class="alert alert-error">No HDD items available in stock for <?= htmlspecialchars($selected_branch) ?> branch.</div>
                ...
                <?php else: ?>
                    <form method="POST">
                        ...
                        <div class="form-group">
                            <label>Select HDD Item</label>
                            <select name="hdd_id" required>
                                <option value="">-- Choose HDD --</option>
                                <?php foreach ($stocks as $s): ?>
                                    <option value="<?= $s['id'] ?>">
                                        <?= htmlspecialchars($s['type']) ?> - <?= htmlspecialchars($s['storage']) ?> (Available: <?= $s['quantity'] ?>)
                                    </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group">
                            ...
                            <input type="number" name="quantity" min="1" required placeholder="Number of HDD units to give">
                        </div>
                        <div class="form-group">
