...
if (isset($_POST['search_serial'])) {
    ...
    elseif (empty($delivered_by)) $error = "Please enter the name of the person delivering the phones.";
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer phones from your own branch.";
    else {
        ...
        if (empty($serials)) $error = "No valid serial numbers found.";
            ...
            $stmt = $conn->prepare("SELECT * FROM phones WHERE serial_number IN ($placeholders) AND status = 'instock' AND branch = ?");
            $params = $serials;
            $params[] = $from_branch;
            $stmt->execute($params);
            $foundPhones = $stmt->fetchAll(PDO::FETCH_ASSOC);
            $foundSerials = array_column($foundPhones, 'serial_number');
            $notFoundSerials = array_diff($serials, $foundSerials);

            if (empty($foundPhones)) $error = "Phones not found in selected branch or not in stock.";
            elseif (count($serials) === 1 && !empty($foundPhones)) {
                $singlePhone = $foundPhones[0];
                $foundPhones = [];
            }
...
if (isset($_POST['transfer_phone'])) {
    ...
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer phones from your own branch.";
        ...
        $stmt = $conn->prepare("SELECT * FROM phones WHERE serial_number = ? AND status = 'instock' AND branch = ?");
        $stmt->execute([$serial, $from_branch]);
        $phone = $stmt->fetch(PDO::FETCH_ASSOC);
        if ($phone) {
            $update = $conn->prepare("UPDATE phones SET branch = ? WHERE serial_number = ?");
            $update->execute([$to_branch, $serial]);
            $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Transfer phone', ?)");
            $log->execute([$user_id, "Transferred phone SN: {$phone['serial_number']} from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
        ...
        } else $error = "Phone not found in selected branch or already sold.";
    }
}

if (isset($_POST['transfer_bulk_phones'])) {
    $selectedSerials = $_POST['selected_serials'] ?? [];
    $from_branch = $_SESSION['transfer_from_branch'] ?? null;
    $to_branch = $_SESSION['transfer_to_branch'] ?? null;
    $delivered_by = $_SESSION['delivered_by'] ?? '';
    if (empty($selectedSerials)) $error = "No phones selected.";
    ...
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer phones from your own branch.";
        ...
        foreach ($selectedSerials as $serial) {
            ...
            if ($stmt->fetch()) {
                ...
                $update->execute([$to_branch, $serial]);
                $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Transfer phone', ?)");
                $log->execute([$user_id, "Transferred phone SN: $serial from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
                $transferredCount++;
        ...
        if ($transferredCount > 0) {
            ...
            $log->execute([$user_id, "Bulk transfer: $transferredCount phone(s) [SN: " . implode(', ', $transferredSerials) . "] from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
            $success = "$transferredCount phone(s) transferred successfully!";
            $foundPhones = [];
            unset($_SESSION['transfer_from_branch'], $_SESSION['transfer_to_branch'], $_SESSION['delivered_by']);
        } else $error = "No phones could be transferred.";
    }
}
...
<head>
    ...
    <title>Transfer Phone | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-mobile-alt"></i> Transfer Phone</h1>
    ...
    <div class="form-container">
        ...
        <form method="POST">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <button type="submit" name="search_serial"><i class="fas fa-search"></i> Search Phones</button>
                </div>
            </div>
        </form>
        ...
        <?php if ($singlePhone): ?>
            <form method="POST">
                <div class="card">
                    <div class="card-header"><h2><i class="fas fa-mobile-alt"></i> Phone Details</h2></div>
                    <div class="card-body">
                        <table>
                            <tr><th>Serial Number</th><td><?= htmlspecialchars($singlePhone['serial_number']) ?></td></tr>
                            <tr><th>Brand</th><td><?= htmlspecialchars($singlePhone['brand']) ?></td></tr>
                            <tr><th>Model</th><td><?= htmlspecialchars($singlePhone['model']) ?></td></tr>
                            <tr><th>RAM</th><td><?= $singlePhone['ram'] ?> GB</td></tr>
                            <tr><th>Storage</th><td><?= $singlePhone['storage_capacity'] ?> GB</td></tr>
                            <tr><th>Condition</th><td><?= htmlspecialchars($singlePhone['phone_condition']) ?></td></tr>
                            <tr><th>Current Branch</th><td><?= htmlspecialchars($singlePhone['branch']) ?></td></tr>
                        ...
                        <input type="hidden" name="serial_number" value="<?= htmlspecialchars($singlePhone['serial_number']) ?>">
                        <button type="submit" name="transfer_phone"><i class="fas fa-arrow-right"></i> Confirm Transfer</button>
                    </div>
                </div>
            </form>
        <?php endif; ?>

        <?php if (!empty($foundPhones)): ?>
            <form method="POST">
                <div class="card">
                    <div class="card-header"><h2><i class="fas fa-list"></i> Found Phones (<?= count($foundPhones) ?>)</h2></div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table>
                                <thead><tr><th class="checkbox-cell">#</th><th class="checkbox-cell">Transfer</th><th>Serial</th><th>Brand</th><th>Model</th><th>RAM</th><th>Storage</th><th>Condition</th><th>Current Branch</th></tr></thead>
                                <tbody>
                                <?php foreach ($foundPhones as $idx => $p): ?>
                                    <tr>
                                        ...
                                        <td><?= htmlspecialchars($p['phone_condition']) ?></td>
                        ...
                        <button type="submit" name="transfer_bulk_phones" style="margin-top:1rem"><i class="fas fa-arrow-right"></i> Transfer Selected (<?= count($foundPhones) ?>)</button>
                    </div>
                </div>
            </form>
        <?php endif; ?>
