...
if ($search_sn) {
    $sql .= " AND s.serial_number LIKE :sn";
    $params['sn'] = "%$search_sn%";
}
if ($search_model) {
    $sql .= " AND s.model LIKE :model";
...
if ($start_date && $end_date) {
    $sql .= " AND DATE(s.sold_at) BETWEEN :start AND :end";
...
$total_sold = count($smartboards);
$total_revenue = array_sum(array_column($smartboards, 'selling_price'));
?>
...
<head>
    ...
    <title>Sold Smartboards | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* Same CSS as sold_devices.php – copy exactly */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-chalkboard"></i> Sold Smartboards</h1>
    ...
    <div class="stats-row">
        ...
        <div class="stat-card">
            ...
            <div class="stat-value">KES <?= $total_sold > 0 ? number_format($total_revenue / $total_sold, 0) : 0 ?></div>
            <div class="stat-label">Average Price</div>
        </div>
    </div>

    <div class="search-section">
        <div class="search-title"><i class="fas fa-filter"></i> Filter Sold Smartboards</div>
        <form method="GET" class="search-grid">
            <div class="search-group">
                <label>Serial Number</label>
                <input type="text" name="sn" placeholder="Scan or type serial" value="<?= htmlspecialchars($search_sn) ?>">
            </div>
            <div class="search-group">
                <label>Model</label>
            ...
            <div class="search-actions">
                ...
                <a href="sold_smartboards.php" class="btn btn-secondary"><i class="fas fa-undo"></i> Reset</a>
            </div>
        </form>
    </div>
                ...
                <table>
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td><span class="price">KES <?= number_format($s['selling_price'], 0) ?></span></td>
                            <td><?= htmlspecialchars($s['sold_by_name'] ?? 'Unknown') ?></td>
                            <td><small><?= date('M j, Y H:i', strtotime($s['sold_at'])) ?></small></td>
                            <td><a href="view_smartboard.php?sn=<?= urlencode($s['serial_number']) ?>" class="action-link"><i class="fas fa-eye"></i> View</a></td>
                        </tr>
                        <?php endforeach; ?>
                    </tbody>
                </table>
