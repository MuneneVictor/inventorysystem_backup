...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $phone_condition = $_POST['phone_condition'] ?? null;
    $added_by      = (int) $_SESSION['user_id'];
    $status        = 'instock'; // default

    // Validation
    if (empty($serial_number) || empty($brand) || empty($model) || $ram <= 0 || $storage_capacity <= 0 || empty($branch)) {
        $error = "Serial Number, Brand, Model, RAM (positive integer), Storage Capacity (positive integer), and Branch are required.";
    } else {
        // Check for duplicate serial number
        $stmt = $conn->prepare("SELECT COUNT(*) FROM phones WHERE serial_number = :sn");
        $stmt->execute(['sn' => $serial_number]);
        if ($stmt->fetchColumn() > 0) {
            $error = "A phone with this serial number already exists!";
        } else {
            try {
                $insert = $conn->prepare("
                    INSERT INTO phones 
                    (serial_number, brand, model, ram, storage_capacity, status, branch, added_by, price, phone_condition)
                    VALUES (:serial_number, :brand, :model, :ram, :storage_capacity, :status, :branch, :added_by, :price, :phone_condition)
                ");
                $insert->execute([
                ...
                // Log activity
                $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Added Phone', :details)");
                $log->execute([
                    'uid'     => $_SESSION['user_id'],
                    'details' => "Added Phone SN: $serial_number (Brand: $brand, Model: $model, RAM: {$ram}GB, Storage: {$storage_capacity}GB, Branch: $branch)"
                ]);
...
<head>
    ...
    <title>Add Phone | Mombasa Computers</title>
    ...
    <div class="form-container">
        ...
        <div class="form-body">
            ...
            <form method="POST" id="phoneForm">
                <div class="form-grid">
                    ...
                    <div class="form-group">
                        ...
                        <input type="text" name="brand" required placeholder="e.g., Apple, Samsung, Nokia">
                    </div>

                    <!-- Model -->
                    <div class="form-group">
                        <label>Model <span class="required">*</span></label>
                        <input type="text" name="model" required placeholder="e.g., iPhone 15 Pro, Galaxy S24">
                    </div>

                    <!-- RAM -->
                    <div class="form-group">
                        <label>RAM (GB) <span class="required">*</span></label>
                        <input type="number" name="ram" min="1" required placeholder="e.g., 4, 6, 8">
                    </div>

                    <!-- Storage Capacity -->
                    <div class="form-group">
                        <label>Storage (GB) <span class="required">*</span></label>
                        <input type="number" name="storage_capacity" min="1" required placeholder="e.g., 64, 128, 256">
                    </div>
                    ...
                    <!-- Phone Condition -->
                    <div class="form-group">
                        <label>Phone Condition <span class="optional">(optional)</span></label>
                ...
                <div class="form-actions">
                    ...
                    <button type="submit" class="btn btn-primary">
                        <i class="fas fa-save"></i> Add Phone
                    </button>
                </div>
            </form>
            ...
            <div class="note-box">
                ...
                <p><strong>Note:</strong> All newly added phones are automatically set to <strong>"In Stock"</strong> status.</p>
            </div>
        </div>
    </div>
