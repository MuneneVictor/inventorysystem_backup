...
if (isset($_POST['search_serial'])) {
    ...
    elseif (empty($delivered_by)) $error = "Please enter the name of the person delivering the UPS units.";
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer UPS units from your own branch.";
    else {
        ...
        if (empty($serials)) $error = "No valid serial numbers found.";
            ...
            $stmt = $conn->prepare("SELECT * FROM ups WHERE serial_number IN ($placeholders) AND status = 'instock' AND branch = ?");
            $params = $serials;
            $params[] = $from_branch;
            $stmt->execute($params);
            ...
            if (empty($foundUPS)) $error = "UPS units not found in selected branch or not in stock.";
...
if (isset($_POST['transfer_ups'])) {
    ...
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer UPS units from your own branch.";
        ...
        if ($ups) {
            $update = $conn->prepare("UPDATE ups SET branch = ? WHERE serial_number = ?");
            $update->execute([$to_branch, $serial]);
            $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Transfer UPS', ?)");
            $log->execute([$user_id, "Transferred UPS SN: {$ups['serial_number']} from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
            $success = "UPS unit transferred successfully!";
            $singleUPS = null;
            unset($_SESSION['transfer_from_branch'], $_SESSION['transfer_to_branch'], $_SESSION['delivered_by']);
        } else $error = "UPS unit not found in selected branch or already sold.";
    }
}

if (isset($_POST['transfer_bulk_ups'])) {
    $selectedSerials = $_POST['selected_serials'] ?? [];
    $from_branch = $_SESSION['transfer_from_branch'] ?? null;
    $to_branch = $_SESSION['transfer_to_branch'] ?? null;
    $delivered_by = $_SESSION['delivered_by'] ?? '';
    if (empty($selectedSerials)) $error = "No UPS units selected.";
    ...
    elseif (!$is_super_admin && $from_branch !== $user_branch) $error = "You can only transfer UPS units from your own branch.";
        ...
        foreach ($selectedSerials as $serial) {
            ...
            if ($stmt->fetch()) {
                ...
                $update->execute([$to_branch, $serial]);
                $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (?, 'Transfer UPS', ?)");
                $log->execute([$user_id, "Transferred UPS SN: $serial from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
                $transferredCount++;
        ...
        if ($transferredCount > 0) {
            ...
            $log->execute([$user_id, "Bulk transfer: $transferredCount UPS unit(s) [SN: " . implode(', ', $transferredSerials) . "] from $from_branch to $to_branch (Delivered by: $delivered_by)"]);
            $success = "$transferredCount UPS unit(s) transferred successfully!";
            $foundUPS = [];
            unset($_SESSION['transfer_from_branch'], $_SESSION['transfer_to_branch'], $_SESSION['delivered_by']);
        } else $error = "No UPS units could be transferred.";
    }
}
...
<head>
    ...
    <title>Transfer UPS | Mombasa Computers</title>
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-battery-three-quarters"></i> Transfer UPS</h1>
    ...
    <div class="form-container">
        ...
        <form method="POST">
            <div class="card">
                ...
                <div class="card-body">
                    ...
                    <button type="submit" name="search_serial"><i class="fas fa-search"></i> Search UPS Units</button>
                </div>
            </div>
        </form>
        ...
        <?php if ($singleUPS): ?>
            <form method="POST">
                <div class="card">
                    <div class="card-header"><h2><i class="fas fa-battery-three-quarters"></i> UPS Details</h2></div>
                    <div class="card-body">
                        <table>
                            <tr><th>Serial Number</th><td><?= htmlspecialchars($singleUPS['serial_number']) ?></td></tr>
                            <tr><th>Model</th><td><?= htmlspecialchars($singleUPS['model']) ?></td></tr>
                            <tr><th>Capacity</th><td><?= $singleUPS['capacity'] ?> VA</td></tr>
                            <tr><th>Condition</th><td><?= htmlspecialchars($singleUPS['ups_condition']) ?></td></tr>
                            <tr><th>Current Branch</th><td><?= htmlspecialchars($singleUPS['branch']) ?></td></tr>
                        ...
                        <button type="submit" name="transfer_ups"><i class="fas fa-arrow-right"></i> Confirm Transfer</button>
                    </div>
                </div>
            </form>
        <?php endif; ?>
        ...
        <?php if (!empty($foundUPS)): ?>
            <form method="POST">
                <div class="card">
                    <div class="card-header"><h2><i class="fas fa-list"></i> Found UPS Units (<?= count($foundUPS) ?>)</h2></div>
                    <div class="card-body">
                        ...
                        <div class="table-responsive">
                            <table>
                                <thead><tr><th class="checkbox-cell">#</th><th class="checkbox-cell">Transfer</th><th>Serial</th><th>Model</th><th>Capacity</th><th>Condition</th><th>Current Branch</th></tr></thead>
                                <tbody>
                                <?php foreach ($foundUPS as $idx => $u): ?>
                                    <tr>
                                        ...
                                        <td><?= $u['capacity'] ?> VA</td>
                                        <td><?= htmlspecialchars($u['ups_condition']) ?></td>
                        ...
                        <button type="submit" name="transfer_bulk_ups" style="margin-top:1rem"><i class="fas fa-arrow-right"></i> Transfer Selected (<?= count($foundUPS) ?>)</button>
                    </div>
                </div>
            </form>
        <?php endif; ?>
