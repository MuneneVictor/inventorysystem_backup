...
function buildMonitorSpecs($monitor) {
    $specs = "";
    if (!empty($monitor['model_name'])) $specs .= $monitor['model_name'];
    if (!empty($monitor['size_inches'])) $specs .= " | " . $monitor['size_inches'] . ' inch';
    if (!empty($monitor['monitor_condition'])) $specs .= " | " . $monitor['monitor_condition'];
    return trim($specs, " |");
}
    ...
    if (empty($input)) {
        ...
        if (empty($serials)) {
            ...
            if (empty($foundMonitors)) {
                $error = "Monitors not found, not in stock, or not in your branch.";
...
if (isset($_POST['sell_monitor'])) {
    ...
    if ($selling_price === '' || !is_numeric($selling_price) || $selling_price <= 0) {
        ...
        try {
            ...
            if (!$monitor) {
                throw new Exception("Monitor not found in your branch or already sold.");
            }
            ...
            $specs = buildMonitorSpecs($monitor);
            ...
            $log->execute([$user_id, "Sold monitor SN: $serial for KES " . number_format($selling_price, 2) . " in sale #$sale_id"]);

            $conn->commit();
            header("Location: checkout.php?sale_id=$sale_id&success=monitor_sold");
            exit;
        } catch (Exception $e) {
            $conn->rollBack();
    ...
    if (empty($selectedSerials)) {
        ...
        foreach ($selectedSerials as $serial) {
            ...
            if ($price === '' || !is_numeric($price) || $price <= 0) {
                $error = "Please enter a valid selling price for all selected monitors.";
                break;
            }
            $prices[$serial] = $price;
        }
        ...
        if (!$error) {
            ...
            try {
                ...
                if ($soldCount > 0) {
                    header("Location: checkout.php?sale_id=$sale_id&success=bulk_monitors_sold");
                    exit;
                } else {
                    $error = "No monitors could be sold.";
                }
            } catch (Exception $e) {
                $conn->rollBack();
                $error = "Error: " . $e->getMessage();
...
<div class="main-content">
    ...
    <div class="card">
        <div class="card-header"><i class="fas fa-search"></i> Search Monitors</div>
        <div class="card-body">
            <form method="POST">
                ...
                <button type="submit" name="search_serial" class="btn"><i class="fas fa-search"></i> Search Monitor(s)</button>
            </form>
    ...
    <?php if ($singleMonitor): ?>
        <div class="card">
            ...
            <div class="card-body">
                <table>
                    <tr><th>Serial</th><td><?= htmlspecialchars($singleMonitor['serial_number']) ?></td></tr>
                    <tr><th>Model</th><td><?= htmlspecialchars($singleMonitor['model_name']) ?></td></tr>
                    <tr><th>Size</th><td><?= $singleMonitor['size_inches'] ?> inches</td></tr>
                    <?php if (!empty($singleMonitor['monitor_condition'])): ?>
                        <tr><th>Condition</th><td><?= htmlspecialchars($singleMonitor['monitor_condition']) ?></td></tr>
                    <?php endif; ?>
                    <tr><th>Branch</th><td><?= htmlspecialchars($singleMonitor['branch']) ?></td></tr>
                    <tr>
                        ...
                        <td>
                            ...
                            <?php if ($singleMonitor['price']): ?>
                                <span style="font-size:0.8rem; color:var(--gray-500);"> (suggested: <?= number_format($singleMonitor['price'], 2) ?>)</span>
                            <?php else: ?>
    ...
    <?php if (!empty($foundMonitors)): ?>
        <div class="card">
            <div class="card-header"><i class="fas fa-list"></i> Found Monitors (<?= count($foundMonitors) ?>)</div>
            <div class="card-body">
                <form method="POST">
                    <input type="hidden" name="sale_id" value="<?= $sale_id ?>">
                    ...
                    <div class="table-responsive">
                        <table>
                            ...
                            <tbody>
                                <?php $counter = 1; foreach ($foundMonitors as $m): ?>
                                <tr>
                                    ...
                                    <td>
                                        <span class="specs-text" title="<?= htmlspecialchars(buildMonitorSpecs($m)) ?>">
                                            <?= htmlspecialchars(buildMonitorSpecs($m)) ?>
                                        </span>
                                    </td>
                                    <td class="price-cell">
                                        <input type="number" name="selling_price_<?= htmlspecialchars($m['serial_number']) ?>" step="0.01" min="0.01"
                                               value="<?= $m['price'] ?? '' ?>"
                                               placeholder="Enter price" required>
                                        <?php if ($m['price']): ?>
                                            <span class="suggested">suggested: <?= number_format($m['price'], 2) ?></span>
                                        <?php endif; ?>
                                    </td>
                                </tr>
                                <?php endforeach; ?>
                    ...
                    <button type="submit" name="sell_bulk_monitors" class="btn" style="margin-top:1rem;">Sell Selected</button>
                </form>
            </div>
        </div>
    <?php endif; ?>
