...
// Build condition clause for printer_condition
$cond_clause = $condition ? '= :condition' : 'IS NULL';

// Count how many printers in this group are missing a price
$countStmt = $conn->prepare("
    SELECT COUNT(*) 
    FROM printers 
    WHERE model_name = :model 
      AND printer_condition $cond_clause
      AND status = 'In Stock'
      AND price IS NULL
");
$params = ['model' => $model];
...
// Fetch one sample printer to display specs
...
if (!$sample) {
    die("No printers found matching this group without a price.");
}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!is_numeric($price) || $price <= 0) {
        ...
        if ($apply_to_all) {
            // Update all printers in this group that have no price
            $update = $conn->prepare("
                UPDATE printers
                SET price = :price
                WHERE model_name = :model
            ...
            // Update only the first printer in this group
        ...
        // Log activity
        $log = $conn->prepare("INSERT INTO activity_logs (user_id, action, details) VALUES (:uid, 'Added printer group price', :details)");
        $log->execute([
            'uid' => $_SESSION['user_id'],
            'details' => "Added price KES $price for printer group: $model" . ($condition ? " ($condition)" : "") . " – $affected printers updated"
        ]);
...
<head>
    ...
    <title>Add Printer Group Price | Mombasa Computers</title>
    <link rel="stylesheet" href="https://cdnjs.cloudflare.com/ajax/libs/font-awesome/6.0.0/css/all.min.css">
    <style>
        /* ===== SAME CSS AS add_price_phone_group.php ===== */
        :root {
            --primary: #1a4b2a;
...
<div class="main-content">
    <div class="page-header">
        <h1><i class="fas fa-plus-circle"></i> Add Printer Group Price</h1>
    ...
    <div class="form-container">
        <div class="card">
            <div class="card-header">
                <h2><i class="fas fa-dollar-sign"></i> Set Printer Group Price</h2>
            </div>
            <div class="card-body">
                <?php if ($error): ?>
                ...
                <form method="POST">
                    <div class="specs-box">
                        ...
                        <p><strong>Condition:</strong> <?= htmlspecialchars($sample['printer_condition'] ?? 'N/A') ?></p>
                    </div>

                    <?php if ($total_count > 1): ?>
                        <div class="info-note">
                            <i class="fas fa-info-circle"></i>
                            <strong>Note:</strong> There are <strong><?= $total_count ?></strong> printers in this group that do not have a price.
                        ...
                        <div class="checkbox-group">
                            ...
                            <label for="apply_to_all">Apply this price to ALL <?= $total_count ?> printers with same specifications</label>
                        </div>
                    <?php endif; ?>
